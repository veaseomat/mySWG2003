...
PVPDJKEncounter2 = Encounter:new {
	-- Task properties
	taskName = "PVPDJKEncounter2",
	-- Encounter properties
	encounterDespawnTime = 2 * 60 * 1000, -- 30 sec
	spawnObjectList = {
		{ template = "pvpdjk2", minimumDistance = 64, maximumDistance = 96, referencePoint = 0, followPlayer = true, setNotAttackable = false, runOnDespawn = true },
	},
...
function PVPDJKEncounter2:isTheFirstSithShadowOfThePlayer(pSithShadow, pPlayer)
...
function PVPDJKEncounter2:onLoot(pLootedCreature, pLooter, nothing)
	...
	CreatureObject(pLooter):awardExperience("force_rank_xp", 3500, true)	
...
function PVPDJKEncounter2:onPlayerKilled(pPlayer, pKiller, nothing)
	...
	if SpawnMobiles.isFromSpawn(pPlayer, PVPDJKEncounter2.taskName, pKiller) then
...
--function PVPDJKEncounter2:onsithKilled(pMellichae, pKiller)
...
function PVPDJKEncounter2:onEncounterSpawned(pPlayer, spawnedObjects)
...
function PVPDJKEncounter2:onEncounterInRange(pPlayer, spawnedObjects)
	if (pPlayer == nil or spawnedObjects == nil or spawnedObjects[1] == nil) then
		return
	end
...
function PVPDJKEncounter2:isEncounterFinished(pPlayer)
	if (pPlayer == nil) then
		return false
	end
...
function PVPDJKEncounter2:useWaypointDatapad(pSceneObject, pPlayer)
	

end

function PVPDJKEncounter2:taskFinish(pPlayer)
	if (pPlayer == nil) then
		return true
	end
