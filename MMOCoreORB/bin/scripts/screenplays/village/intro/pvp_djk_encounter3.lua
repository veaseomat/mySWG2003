...
PVPDJKEncounter3 = Encounter:new {
	-- Task properties
	taskName = "PVPDJKEncounter3",
	-- Encounter properties
	encounterDespawnTime = 2 * 60 * 1000, -- 30 sec
	spawnObjectList = {
		{ template = "pvpdjk3", minimumDistance = 64, maximumDistance = 96, referencePoint = 0, followPlayer = true, setNotAttackable = false, runOnDespawn = true },
	},
...
function PVPDJKEncounter3:isTheFirstSithShadowOfThePlayer(pSithShadow, pPlayer)
...
function PVPDJKEncounter3:onLoot(pLootedCreature, pLooter, nothing)
...
function PVPDJKEncounter3:onPlayerKilled(pPlayer, pKiller, nothing)
	...
	if SpawnMobiles.isFromSpawn(pPlayer, PVPDJKEncounter3.taskName, pKiller) then
...
--function PVPDJKEncounter3:onsithKilled(pMellichae, pKiller)
...
function PVPDJKEncounter3:onEncounterSpawned(pPlayer, spawnedObjects)
...
function PVPDJKEncounter3:onEncounterInRange(pPlayer, spawnedObjects)
	if (pPlayer == nil or spawnedObjects == nil or spawnedObjects[1] == nil) then
		return
	end
...
function PVPDJKEncounter3:isEncounterFinished(pPlayer)
	if (pPlayer == nil) then
		return false
	end
...
function PVPDJKEncounter3:useWaypointDatapad(pSceneObject, pPlayer)
	

end

function PVPDJKEncounter3:taskFinish(pPlayer)
	if (pPlayer == nil) then
		return true
	end
