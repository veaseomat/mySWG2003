...
PVPDJKEncounter4 = Encounter:new {
	-- Task properties
	taskName = "PVPDJKEncounter4",
	-- Encounter properties
	encounterDespawnTime = 2 * 60 * 1000, -- 30 sec
	spawnObjectList = {
		{ template = "pvpdjk4", minimumDistance = 64, maximumDistance = 96, referencePoint = 0, followPlayer = true, setNotAttackable = false, runOnDespawn = true },
	},
...
function PVPDJKEncounter4:isTheFirstSithShadowOfThePlayer(pSithShadow, pPlayer)
...
function PVPDJKEncounter4:onLoot(pLootedCreature, pLooter, nothing)
...
function PVPDJKEncounter4:onPlayerKilled(pPlayer, pKiller, nothing)
	...
	if SpawnMobiles.isFromSpawn(pPlayer, PVPDJKEncounter4.taskName, pKiller) then
...
--function PVPDJKEncounter4:onsithKilled(pMellichae, pKiller)
...
function PVPDJKEncounter4:onEncounterSpawned(pPlayer, spawnedObjects)
...
function PVPDJKEncounter4:onEncounterInRange(pPlayer, spawnedObjects)
	if (pPlayer == nil or spawnedObjects == nil or spawnedObjects[1] == nil) then
		return
	end
...
function PVPDJKEncounter4:isEncounterFinished(pPlayer)
	if (pPlayer == nil) then
		return false
	end
...
function PVPDJKEncounter4:useWaypointDatapad(pSceneObject, pPlayer)
	

end

function PVPDJKEncounter4:taskFinish(pPlayer)
	if (pPlayer == nil) then
		return true
	end
