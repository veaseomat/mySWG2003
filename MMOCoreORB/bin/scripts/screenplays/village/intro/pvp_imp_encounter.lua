...
PVPIMPEncounter = Encounter:new {
	-- Task properties
	taskName = "PVPIMPEncounter",
...
function PVPIMPEncounter:isTheFirstSithShadowOfThePlayer(pSithShadow, pPlayer)
...
function PVPIMPEncounter:onLoot(pLootedCreature, pLooter, nothing)
...
function PVPIMPEncounter:onPlayerKilled(pPlayer, pKiller, nothing)
	...
	if SpawnMobiles.isFromSpawn(pPlayer, PVPIMPEncounter.taskName, pKiller) then
...
--function PVPIMPEncounter:onsithKilled(pMellichae, pKiller)
...
function PVPIMPEncounter:onEncounterSpawned(pPlayer, spawnedObjects)
...
function PVPIMPEncounter:onEncounterInRange(pPlayer, spawnedObjects)
	if (pPlayer == nil or spawnedObjects == nil or spawnedObjects[1] == nil) then
		return
	end
...
function PVPIMPEncounter:isEncounterFinished(pPlayer)
	if (pPlayer == nil) then
		return false
	end
...
function PVPIMPEncounter:useWaypointDatapad(pSceneObject, pPlayer)
	

end

function PVPIMPEncounter:taskFinish(pPlayer)
	if (pPlayer == nil) then
		return true
	end
