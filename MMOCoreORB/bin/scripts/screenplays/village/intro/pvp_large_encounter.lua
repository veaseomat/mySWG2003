...
pvplargeencounter = Encounter:new {
	-- Task properties
	taskName = "pvplargeencounter",
...
function pvplargeencounter:isTheFirstSithShadowOfThePlayer(pSithShadow, pPlayer)
...
function pvplargeencounter:onLoot(pLootedCreature, pLooter, nothing)
...
function pvplargeencounter:onPlayerKilled(pPlayer, pKiller, nothing)
	...
	if SpawnMobiles.isFromSpawn(pPlayer, pvplargeencounter.taskName, pKiller) then
...
--function pvplargeencounter:onsithKilled(pMellichae, pKiller)
...
function pvplargeencounter:onEncounterSpawned(pPlayer, spawnedObjects)
...
function pvplargeencounter:onEncounterInRange(pPlayer, spawnedObjects)
	if (pPlayer == nil or spawnedObjects == nil or spawnedObjects[1] == nil) then
		return
	end
...
function pvplargeencounter:isEncounterFinished(pPlayer)
	if (pPlayer == nil) then
		return false
	end
...
function pvplargeencounter:useWaypointDatapad(pSceneObject, pPlayer)
	

end

function pvplargeencounter:taskFinish(pPlayer)
	if (pPlayer == nil) then
		return true
	end
