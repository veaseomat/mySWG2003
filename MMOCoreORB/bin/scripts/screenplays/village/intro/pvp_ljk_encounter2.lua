...
PVPLJKEncounter2 = Encounter:new {
	-- Task properties
	taskName = "PVPLJKEncounter2",
	-- Encounter properties
	encounterDespawnTime = 2 * 60 * 1000, -- 30 sec
	spawnObjectList = {
		{ template = "pvpljk2", minimumDistance = 64, maximumDistance = 96, referencePoint = 0, followPlayer = true, setNotAttackable = false, runOnDespawn = true },
	},
...
function PVPLJKEncounter2:isTheFirstSithShadowOfThePlayer(pSithShadow, pPlayer)
...
function PVPLJKEncounter2:onLoot(pLootedCreature, pLooter, nothing)
...
function PVPLJKEncounter2:onPlayerKilled(pPlayer, pKiller, nothing)
	...
	if SpawnMobiles.isFromSpawn(pPlayer, PVPLJKEncounter2.taskName, pKiller) then
...
--function PVPLJKEncounter2:onsithKilled(pMellichae, pKiller)
...
function PVPLJKEncounter2:onEncounterSpawned(pPlayer, spawnedObjects)
...
function PVPLJKEncounter2:onEncounterInRange(pPlayer, spawnedObjects)
	if (pPlayer == nil or spawnedObjects == nil or spawnedObjects[1] == nil) then
		return
	end
...
function PVPLJKEncounter2:isEncounterFinished(pPlayer)
	if (pPlayer == nil) then
		return false
	end
...
function PVPLJKEncounter2:useWaypointDatapad(pSceneObject, pPlayer)
	

end

function PVPLJKEncounter2:taskFinish(pPlayer)
	if (pPlayer == nil) then
		return true
	end
