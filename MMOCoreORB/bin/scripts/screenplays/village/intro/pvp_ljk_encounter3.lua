...
PVPLJKEncounter3 = Encounter:new {
	-- Task properties
	taskName = "PVPLJKEncounter3",
	-- Encounter properties
	encounterDespawnTime = 2 * 60 * 1000, -- 30 sec
	spawnObjectList = {
		{ template = "pvpljk3", minimumDistance = 64, maximumDistance = 96, referencePoint = 0, followPlayer = true, setNotAttackable = false, runOnDespawn = true },
	},
...
function PVPLJKEncounter3:isTheFirstSithShadowOfThePlayer(pSithShadow, pPlayer)
...
function PVPLJKEncounter3:onLoot(pLootedCreature, pLooter, nothing)
...
function PVPLJKEncounter3:onPlayerKilled(pPlayer, pKiller, nothing)
	...
	if SpawnMobiles.isFromSpawn(pPlayer, PVPLJKEncounter3.taskName, pKiller) then
...
--function PVPLJKEncounter3:onsithKilled(pMellichae, pKiller)
...
function PVPLJKEncounter3:onEncounterSpawned(pPlayer, spawnedObjects)
...
function PVPLJKEncounter3:onEncounterInRange(pPlayer, spawnedObjects)
	if (pPlayer == nil or spawnedObjects == nil or spawnedObjects[1] == nil) then
		return
	end
...
function PVPLJKEncounter3:isEncounterFinished(pPlayer)
	if (pPlayer == nil) then
		return false
	end
...
function PVPLJKEncounter3:useWaypointDatapad(pSceneObject, pPlayer)
	

end

function PVPLJKEncounter3:taskFinish(pPlayer)
	if (pPlayer == nil) then
		return true
	end
