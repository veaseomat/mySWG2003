...
PVPLJKEncounter4 = Encounter:new {
	-- Task properties
	taskName = "PVPLJKEncounter4",
	-- Encounter properties
	encounterDespawnTime = 2 * 60 * 1000, -- 30 sec
	spawnObjectList = {
		{ template = "pvpljk4", minimumDistance = 64, maximumDistance = 96, referencePoint = 0, followPlayer = true, setNotAttackable = false, runOnDespawn = true },
	},
...
function PVPLJKEncounter4:isTheFirstSithShadowOfThePlayer(pSithShadow, pPlayer)
...
function PVPLJKEncounter4:onLoot(pLootedCreature, pLooter, nothing)
	...
	CreatureObject(pLooter):awardExperience("force_rank_xp", 6000, true)	
...
function PVPLJKEncounter4:onPlayerKilled(pPlayer, pKiller, nothing)
	...
	if SpawnMobiles.isFromSpawn(pPlayer, PVPLJKEncounter4.taskName, pKiller) then
...
--function PVPLJKEncounter4:onsithKilled(pMellichae, pKiller)
...
function PVPLJKEncounter4:onEncounterSpawned(pPlayer, spawnedObjects)
...
function PVPLJKEncounter4:onEncounterInRange(pPlayer, spawnedObjects)
	if (pPlayer == nil or spawnedObjects == nil or spawnedObjects[1] == nil) then
		return
	end
...
function PVPLJKEncounter4:isEncounterFinished(pPlayer)
	if (pPlayer == nil) then
		return false
	end
...
function PVPLJKEncounter4:useWaypointDatapad(pSceneObject, pPlayer)
	

end

function PVPLJKEncounter4:taskFinish(pPlayer)
	if (pPlayer == nil) then
		return true
	end
