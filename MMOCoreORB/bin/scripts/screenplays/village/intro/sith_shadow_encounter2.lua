...
sithshadowencounter2 = Encounter:new {
	-- Task properties
	taskName = "sithshadowencounter2",
	-- Encounter properties
	encounterDespawnTime = 2 * 60 * 1000, -- 30 sec
	spawnObjectList = {
		{ template = "dark_jedi_knight2", minimumDistance = 64, maximumDistance = 96, referencePoint = 0, followPlayer = true, setNotAttackable = false, runOnDespawn = true },
	},
...
function sithshadowencounter2:isTheFirstSithShadowOfThePlayer(pSithShadow, pPlayer)
...
function sithshadowencounter2:onLoot(pLootedCreature, pLooter, nothing)
...
function sithshadowencounter2:onPlayerKilled(pPlayer, pKiller, nothing)
	...
	if SpawnMobiles.isFromSpawn(pPlayer, sithshadowencounter2.taskName, pKiller) then
...
--function sithshadowencounter2:onsithKilled(pMellichae, pKiller)
...
function sithshadowencounter2:onEncounterSpawned(pPlayer, spawnedObjects)
...
function sithshadowencounter2:onEncounterInRange(pPlayer, spawnedObjects)
	if (pPlayer == nil or spawnedObjects == nil or spawnedObjects[1] == nil) then
		return
	end
...
function sithshadowencounter2:isEncounterFinished(pPlayer)
	if (pPlayer == nil) then
		return false
	end
...
function sithshadowencounter2:useWaypointDatapad(pSceneObject, pPlayer)
	

end

function sithshadowencounter2:taskFinish(pPlayer)
	if (pPlayer == nil) then
		return true
	end
