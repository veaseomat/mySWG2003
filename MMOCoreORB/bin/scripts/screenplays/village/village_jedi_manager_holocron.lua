...
HOLOCRONCOOLDOWNTIME = 1 * 1000--1sec --12 * 60 * 60 * 1000
...
function VillageJediManagerHolocron.useTheHolocron(pSceneObject, pPlayer)
	...
	PlayerObject(pGhost):setForcePower(PlayerObject(pGhost):getForcePower() + 1000) --PlayerObject(pGhost):getForcePowerMax())--PlayerObject(pGhost):getForcePower() + ((PlayerObject(pGhost):getForcePowerMax() - PlayerObject(pGhost):getForcePower()) / 2));
